...
extract_clean_text <- function(html_string) {
  ...
  # read_html parses the structure and decodes entities (&uuml; -> ü)
  # html_text2 is preferred over html_text as it handles block-level elements (like <br>) better
  parsed_text <- minimal_html(html_string) %>% 
    html_text2()
...
for (i in 1:nrow(data)) {
  data[i,"URI"] <- rdfhelper::uri(toupper(rlang::hash(data[i,"Anforderungen"])), base)
}
